...
public:
    SMTSampler(std::string input, int max_samples, double max_time, int strategy) : input_file(input), max_samples(max_samples), max_time(max_time), strategy(strategy), params(c), opt(c), solver(c), model(c), smt_formula(c) {
    ...
    void run() {
        ...
        while (true) {
            ...
            for (z3::func_decl & v : ind) {
                ...
                {
		    if (random_soft_bit) {
                        for (unsigned i = 0; i < v.range().bv_size(); ++i) {
                            if (rand() % 2)
    ...
    void sample(z3::model m) {
        ...
        for (unsigned count = 0; count < ind.size(); ++count) {
        ...
        int calls = 0;
        int progress = 0;
        for (unsigned count = 0; count < constraints.size(); ++count) {
    ...
    void add_constraints(z3::expr exp, z3::expr val, int count) {
        ...
        {
            std::vector<z3::expr> soft;
            for (unsigned i = 0; i < val.get_sort().bv_size(); ++i) {
    ...
    std::string model_string(z3::model m, std::vector<z3::func_decl> ind) {
        ...
        for (z3::func_decl & v : ind) {
		    ...
		    s += def + '\0';
		    for (unsigned j = 0; j < f.num_entries(); ++j) {
		    ...
		    s += "]";
                ...
                s += def + '\0';
                for (unsigned j = 0; j < f.num_entries(); ++j) {
                    for (unsigned k = 0; k < f.entry(j).num_args(); ++k) {
...
void help() {
   std::cout << "-----------SMTSampler-----------\n";
   std::cout << "./samp -n 10000 -t 3600.0 --smtbit formula.smt2\n";
   std::cout << "-n: number of samples \n";
   std::cout << "-t: total time limit \n";
   std::cout << "strategies: --smtbit, --smtbv, --sat, --region\n";
   exit(0);
}
...
int main(int argc, char * argv[]) {
    ...
    if (strategy == START_REGION) {
        RegionSampler s(argv[argc-1], max_samples, max_time);
        s.run();
